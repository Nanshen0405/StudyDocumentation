...
> 6. InooDB当不走索引的时候，select加锁是锁住了整张表。当走索引的时候，是select加锁是行级锁以及gap锁（涉及到普通非唯一索引用到的）
...
- 假设我们有一个已经写好的 Java 源文件叫 Robot.java

1. 通过**JAVAC** 命令编译器会将 Robot.java 源文件编译为 Robot.class 字节码文件，并将文件保存在当前目录下
2. **JAVA** 命令执行 Robot.class 字节码文件的时候，类加载器 **ClassLoader**会将字节码转换为 JVM 中的*Class<Robot>*对象
3. JVM 会利用 `Class<Robot>` 对象实例化为 Robot 对象
...
![创建对象时GC的流程图](图片/创建对象时GC的流程图.jpg)
...
> 标记-清除算法的执行的过程如下面视频所示

<video src="图片/标记清除.mp4"></video>
...
#### 10.1.2.2、第二代计算机（若干个计算机互联的时代，计算机网络阶段）

> 简介

（1）名称：晶体管计算机（逻辑元件是晶体管）

（2）内存采用磁芯  外存采用 磁盘或磁带 

（3）第二代计算机使用第三代编程语言（高级语言）编写程序。

① 特殊的，使用BASIC语言编写的 源程序 ，必须经过 解释程序 转化为机器语言后才能被计算机执行。

I. 解释程序是语言处理系统，是一个翻译。

II. 解释程序工作特点：解释一句，执行一句，下次再要执行，重新解释。（不会形成 目标程序）与其他高级语言相比执行效率低。

② 其他高级语言（如 C语言，java等）编写的 源程序，必须经过 编译程序 转化为 目标程序 后，才能被计算机执行。

I 编译程序 是语言处理系统，是一个翻译。

II 编译程序，工作特点：一次编译（形成 目标程序），多次执行（之后的执行，无需再编译，只要执行目标程序即可）。与解释程序相比执行效率高。

③ 第三代高级语言，是结构化程序设计语言，或面向对象过程 程序设计语言，与低级语言相比，编写效率高，执行效率低。可移植性，跨平台性好。

（4）第二代计算机 没有 操作系统。



#### 10.1.2.3、 第三代计算机（计算机网络互联阶段，信息高速公路）

（1）名称：集成电路计算机（逻辑元件是集成电路）

（2）内存使用 磁性半导体材料， 沿用至今。 外存 使用 磁盘。

（3）正式出现操作系统。出现多种高级编程语言。

高级程序设计语言的基本组成：数据、运算、控制和运输。
...
①1980年，美国IBM公司推出IBM-PC（个人计算机，微型计算机）等媒体形成，传播信息。

②出现多媒体计算机，利用文字，图形，视频，动画。等媒体形式，传播信息。

③1985年，出现了Internet（互联网）④出现 超级计算机（超算）

美国“顶点”，中国“神威 太湖之光”，“银河”，“天河”，“天河二号”运算速度达到“亿亿次每秒”
